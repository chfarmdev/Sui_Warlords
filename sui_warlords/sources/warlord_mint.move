...
module sui_warlords::warlord_mint {
    ...
    // Level up section, costs 2 SUI, and allows up to 9 level ups for a max level of 10.
    ...
    // Maybe I could introduce a required "token" to be created for the class change? That token would dictate the mutated class then.

    const WARLORD_CLASS_CHANGE_COST: u64 = 10000000000;
    const WARLORD_IS_WRONG_CLASS: u64 = 3;

    public entry fun cc_warrior_to_berserker(
        warlord: &mut SuiWarlordNFT,
        mut payment: Coin<SUI>,
        newclass: u64;
        ctx: &mut TxContext,
    ...
    // Gotta correct that in half dozen places for linkage and update use commands in a few places
    // 
